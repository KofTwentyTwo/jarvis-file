# Jarvis File — Open Source Interactive Design Document Platform
...
**License**: MIT
**Package**: `npx create-jarvis-file`
...
Jarvis File extracts this into a reusable open source product. Any engineering team can run `npx create-jarvis-file my-project` and get a fully functional, AI-assisted interactive design document site deployed to their own Firebase project in under 30 minutes.

This is **Approach 1** from the [reusability analysis](2026-04-03-jarvis-page-reusability-analysis.md) — config-driven static site with a CLI generator — renamed from "Jarvis Page" to "Jarvis File."
...
## 1. CLI Generator (`create-jarvis-file`)

### 1.1 Package Identity

```
@koftwentytwo/create-jarvis-file
```

Invoked via:
```bash
npx create-jarvis-file my-project
# or
...
```
my-project/
  jarvis-file.config.js        # Single source of truth
  ...
  js/
    auth-gate.js                # Authentication gate (reads window.JF)
    comments.js                 # Comment system (reads window.JF)
    jarvis-chat.js              # Chat assistant (reads window.JF)
    search.js                   # Full-text search (reads window.JF)
    version-loader.js           # Version dropdown (reads window.JF)
    built-with.js               # "Built with Jarvis File" attribution
    config-loader.js            # Loads jarvis-file.config.js → window.JF
  css/
    jarvis-file.css             # Complete design system stylesheet
  jarvis/
    jarvis.js                   # Comment monitor + chat responder
    change-processor.js         # Claude Agent SDK auto-implementation
    jarvis-file.config.json     # Node.js config (generated from .js config)
...
```
Your Jarvis File project is ready.
...
**Browser (client-side)**: A small `config-loader.js` script is included before all other scripts. It loads `jarvis-file.config.js` which sets `window.JF`. All framework JS reads from `window.JF` at runtime.

```html
<!-- In every HTML page <head> -->
<script src="jarvis-file.config.js"></script>
<script src="js/auth-gate.js"></script>
```

**Node.js (backend)**: The CLI generates a parallel `jarvis/jarvis-file.config.json` from the same source values. Backend services read this JSON file at startup.

```javascript
// jarvis/jarvis.js
const config = require("./jarvis-file.config.json");
...
These are the client-side JavaScript files extracted from the current Praesidium2 implementation. Each is refactored to read all project-specific values from `window.JF` instead of hardcoded constants.
...
// After (config-driven)
const AUTH_GATE_CONFIG = window.JF.firebase;
const AUTH_ALLOWED_DOMAIN = window.JF.auth.allowedDomain;
logoDiv.textContent = window.JF.project.logoLetter;
title.textContent = window.JF.project.name;
subtitle.textContent = window.JF.project.subtitle;
```

Primary color for the sign-in button and logo gradient reads from `window.JF.theme.primaryColor` and `window.JF.theme.accentColor`.
...
// After
var FIREBASE_CONFIG = window.JF.firebase;
var ALLOWED_DOMAIN = window.JF.auth.allowedDomain;
var MENTION_USERS = window.JF.team;
```
...
```javascript
var COMMENTS_COLLECTION = window.JF.project.slug + "_comments";
var TYPING_COLLECTION = window.JF.project.slug + "_typing_indicators";
```

localStorage keys are namespaced:
```javascript
var STORAGE_PREFIX = "jf_" + window.JF.project.slug + "_";
```
...
// After
var JARVIS_EMAIL = window.JF.assistant.email;
var CHAT_COLLECTION = window.JF.project.slug + "_jarvis_chats";

// Link map built from config (regex compiled at init time, not per-message)
var LINK_MAP = window.JF.linkMap.map(function(entry) {
  return { re: new RegExp(escapeRegExp(entry.pattern), 'g'), href: entry.href };
});

var WELCOME_MESSAGES = window.JF.welcome.messages.map(function(msg) {
  return {
    content: msg.content.replace(/\{\{PROJECT_NAME\}\}/g, window.JF.project.name),
    roleButtons: msg.roleButtons || null
  };
});

var ROLE_RESPONSES = window.JF.welcome.roleResponses;
```

Session and chat history localStorage keys use the project slug prefix:
```javascript
sessionStorage.getItem("jf_" + window.JF.project.slug + "_session_id");
localStorage.getItem("jf_" + window.JF.project.slug + "_chat_history");
```
...
// After:
"Type to search across all " + window.JF.project.name + " documents"
```
...
**Refactored behavior**: Becomes a generic "Built with Jarvis File" attribution. The modal content is simplified to describe Jarvis File itself (the platform), not any specific project.

```javascript
var MODAL_CONTENT = {
  title: "Built with Jarvis File",
  subtitle: "An open source platform for interactive AI-assisted design documents.",
  sections: [
    {
      heading: "What is Jarvis File?",
      text: "Jarvis File is an open source toolkit by KofTwentyTwo that turns static design documents into interactive, AI-assisted review environments. Comments, search, version tracking, and a real-time AI assistant — all deployed to your own Firebase project."
    },
    {
      heading: "Features",
      items: [
    ...
    {
      heading: "Learn More",
      text: "github.com/KofTwentyTwo/jarvis-file"
    }
  ]
};
```

The footer link text reads "Built with Jarvis File" (always, not configurable — this is the attribution for the open source project).
...
**Persona loading**: Reads `PERSONA.md` (generated from template during `create-jarvis-file`). The persona file uses `{{PROJECT_NAME}}` and `{{ASSISTANT_NAME}}` tokens that are replaced at generation time.
...
<head>
  ...
  <script src="jarvis-file.config.js"></script>
  <script src="js/auth-gate.js"></script>
  <link rel="stylesheet" href="css/jarvis-file.css">
</head>
<body>
  ...
  <script>
    document.getElementById('jf-logo').textContent = window.JF.project.logoLetter;
    document.getElementById('jf-project-name').textContent = window.JF.project.name;
    // Doc nav built from window.JF.docs
  </script>
</body>
</html>
```
...
### 7.1 Extracted Stylesheet: `jarvis-file.css`
...
Theme customization via CSS custom properties, set by `config-loader.js` from `window.JF.theme`:

```css
:root {
  /* Set dynamically from window.JF.theme */
  --jf-primary: #f59e0b;
  --jf-accent: #ef4444;
  --jf-highlight: #22d3ee;
...
Jarvis File v1 ships with the dark theme only. A light theme is a v2 consideration. The dark theme is the product's visual identity.

---

## 8. Hardcoded-to-Configurable Map

Complete inventory of every hardcoded value in the current Praesidium2 codebase and its migration path. Referenced from the [reusability analysis](2026-04-03-jarvis-page-reusability-analysis.md).

| # | Current Value | File(s) | Config Path |
|---|---------------|---------|-------------|
| 1 | Firebase API key | `auth-gate.js:11`, `comments.js:19` | `window.JF.firebase.apiKey` |
| 2 | Firebase project ID | `auth-gate.js:13`, `comments.js:21`, `jarvis.js:24`, `change-processor.js:58` | `window.JF.firebase.projectId` |
| 3 | Firebase auth domain | `auth-gate.js:12`, `comments.js:20` | `window.JF.firebase.authDomain` |
| 4 | Firebase storage bucket | `auth-gate.js:14`, `comments.js:22` | `window.JF.firebase.storageBucket` |
| 5 | Firebase messaging sender ID | `auth-gate.js:15`, `comments.js:23` | `window.JF.firebase.messagingSenderId` |
| 6 | Firebase app ID | `auth-gate.js:16`, `comments.js:24` | `window.JF.firebase.appId` |
| 7 | Allowed domain (`dmdbrands.com`) | `auth-gate.js:19`, `comments.js:27`, `firestore.rules` (x6) | `window.JF.auth.allowedDomain` |
| 8 | Owner email (`jmaes@dmdbrands.com`) | `firestore.rules:51` | `window.JF.auth.ownerEmail` |
| 9 | Project name (`Praesidium2`) | `auth-gate.js:43`, `comments.js:1`, `search.js:211,266,413` | `window.JF.project.name` |
| 10 | Subtitle (`DMD Brands — Confidential...`) | `auth-gate.js:48` | `window.JF.project.subtitle` |
| 11 | Logo letter (`P`) | `auth-gate.js:38` | `window.JF.project.logoLetter` |
| 12 | Jarvis email | `jarvis-chat.js:23`, `jarvis.js:31`, `change-processor.js:98` | `window.JF.assistant.email` |
| 13 | Jarvis author name + photo | `jarvis.js:29-33`, `change-processor.js:96-100` | `window.JF.assistant.{name,avatarUrl}` |
| 14 | DOCS array (search) | `search.js:11-16` | `window.JF.docs` |
| 15 | LINK_MAP (chat links) | `jarvis-chat.js:34-45` | `window.JF.linkMap` |
| 16 | WELCOME_MESSAGES | `jarvis-chat.js:989-1007` | `window.JF.welcome.messages` |
| 17 | ROLE_RESPONSES | `jarvis-chat.js:1009-1018` | `window.JF.welcome.roleResponses` |
| 18 | MENTION_USERS (team directory) | `comments.js:32-40` | `window.JF.team` |
| 19 | DOCUMENT_FILE_MAP | `change-processor.js:80-86` | `docs-manifest.json` |
| 20 | MODAL_CONTENT (built-with) | `built-with.js:9-51` | Static "Built with Jarvis File" |
...
| 25 | MARKDOWN_SPEC_PATH | `change-processor.js:89` | Optional in `jarvis-file.config.json` |
...
jarvis-file/
  packages/
    create-jarvis-file/              # CLI generator (npm package)
      bin/
        create-jarvis-file.js        # Entry point (#!/usr/bin/env node)
      lib/
      ...
      templates/
        jarvis-file.config.js.tmpl   # Config file template
        jarvis-file.config.json.tmpl # Node.js config template
    ...
    jarvis-file-core/                # Framework JS (browser)
      js/
        ...
        config-loader.js             # Loads config → window.JF
      css/
        jarvis-file.css              # Complete design system
      package.json

    jarvis-file-backend/             # Jarvis + change processor (Node.js)
      jarvis.js                      # Comment monitor + chat responder
      change-processor.js            # Claude Agent SDK auto-implementation
      lib/
  ...
  examples/
    praesidium2/                     # The original as reference implementation
      jarvis-file.config.js          # Real config (sanitized credentials)
...
```json
{
  ...
  "private": true,
  "workspaces": [
    "packages/create-jarvis-file",
    "packages/jarvis-file-core",
    "packages/jarvis-file-backend"
  ]
}
```
...
|---------|----------|---------|
| `create-jarvis-file` | `@koftwentytwo/create-jarvis-file` | CLI generator |
| `jarvis-file-core` | `@koftwentytwo/jarvis-file-core` | Browser framework JS + CSS |
| `jarvis-file-backend` | `@koftwentytwo/jarvis-file-backend` | Node.js AI services |

The CLI copies files from `jarvis-file-core` and `jarvis-file-backend` into the generated project. Users do NOT install these packages as dependencies — they get standalone copies they can modify.
...
Extract all hardcoded values into `jarvis-file.config.js` and refactor framework JS to read from `window.JF`.

**Tasks**:
1. Create `jarvis-file.config.js` with the full schema (Section 2.1)
...
7. Refactor `built-with.js` — replace with generic Jarvis File attribution
8. Extract CSS into `jarvis-file.css` from inline `<style>` blocks; convert hardcoded colors to CSS custom properties
9. Refactor `jarvis.js` — read from `jarvis-file.config.json`, template system prompt
10. Refactor `change-processor.js` — read from `jarvis-file.config.json`, build document map from manifest
...
**Exit criteria**: Praesidium2 design site deploys and functions with zero behavioral changes, all config read from `jarvis-file.config.js`.

### Phase 2: CLI Generator (3-4 days)

Build `create-jarvis-file` as an interactive Node.js CLI.

**Tasks**:
1. Set up `packages/create-jarvis-file` with `bin` entry point
...
**Exit criteria**: `npx create-jarvis-file test-project` produces a deployable site with working auth, comments, search, and AI assistant.
...
6. Extract and consolidate CSS into `jarvis-file.css` with CSS custom properties
...
**Tasks**:
1. Create `github.com/KofTwentyTwo/jarvis-file` repository
2. Set up GitHub Actions CI (lint, test)
3. Set up GitHub Actions publish workflow (npm publish on release tag)
4. Publish `@koftwentytwo/create-jarvis-file` to npm
5. Publish `@koftwentytwo/jarvis-file-core` to npm
6. Publish `@koftwentytwo/jarvis-file-backend` to npm
7. Create GitHub release with changelog
8. Add repo topics, description, social preview image
9. Test `npx @koftwentytwo/create-jarvis-file` from a clean machine

**Exit criteria**: `npx @koftwentytwo/create-jarvis-file my-project` works from any machine with Node.js 18+.
...
Each Jarvis File instance gets its own Firebase project, providing complete data isolation.
...
The current Praesidium2 implementation uses global localStorage keys like `jarvis_chat_history`. If two Jarvis File instances run on the same domain (e.g., during local dev on localhost), they would overwrite each other's state. The fix is project-slug prefixing on all storage keys: `jf_{slug}_chat_history`.
...
The `built-with.js` component renders a "Built with Jarvis File" footer on every page. This is the only attribution requirement, and it is not enforced — users can remove it. The footer links to the GitHub repo.

### 14.3 Contribution Model

- Issues and PRs welcome on `github.com/KofTwentyTwo/jarvis-file`
...
1. `npx @koftwentytwo/create-jarvis-file test-project` works on a clean machine with Node.js 18+
