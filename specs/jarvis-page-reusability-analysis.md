# Jarvis Page System — Reusability Assessment
...
JS files read config from `jarvis-page.config.json` at runtime. The "template" IS the source. Generated instances are just config files + content.
...
**Phase 1**: Config externalization — `jarvis-page.config.js` → `window.JPC` global (1 week)
**Phase 2**: Firestore rules templating — build step fills tokens (2 days)
**Phase 3**: Instance generator CLI — `npx create-jarvis-page my-project` (2-3 days)
